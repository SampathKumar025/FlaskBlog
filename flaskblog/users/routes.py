...
def logout():
    logout_user()
    flash(f"You got Logged out!", "danger")
    return redirect(url_for("main.home"))
...
def request_reset():
    ...
    if form.validate_on_submit():
        ...
        flash("A reset link hs been sent to {user.email} with instructions. Check your mail!", "info")
        return redirect(url_for("users.login"))
...
def password_reset(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"Your password has been updated successfully! You can Login now.", "success")
        return redirect(url_for("users.login"))
